...
1. median values of temperature, wind speed, and atmospheric pressure across all stations.
```{r medians-medians}
medians <- met[, .(
...
```{r}
station_med[, temp_50 := quantile(temp, 0.5, na.rm = TRUE), by = STATE]
station_med[, wind.sp_50 := quantile(wind.sp, 0.5, na.rm = TRUE), by = STATE]
...
```{r}
mid_point <- met[, .(
  lon_50 = quantile(lon, 0.5, na.rm = TRUE),
  lat_50 = quantile(lat, 0.5, na.rm = TRUE)
), by = STATE]
